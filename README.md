...
> **Not affiliated with Recordly.** Recordly is made by webadderall and licensed under AGPLv3. This repo contains only an install script. It downloads the **official** Recordly AppImage from Recordly's GitHub release, verifies its checksum, applies three small fixes, and adds a launcher. Once Recordly ships these fixes, uninstall this and use the official release.
...
## Still "could not find video source"? (GPU)

If recording fails with *"could not find video source"* and running the launcher from a terminal prints `Exiting GPU process due to errors during initialization`, Chromium couldn't start its GPU process. This was seen with the NVIDIA proprietary driver (`nv_gbm_create_device_native failed`). Reinstall with software rendering:
